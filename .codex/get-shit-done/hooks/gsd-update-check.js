...
// Detect runtime config directory (supports Codex, OpenCode, Claude)
// Respects CODEX_HOME and CLAUDE_CONFIG_DIR for custom config directory setups
function detectConfigDir(baseDir) {
  // Check env override first (supports multi-account setups)
  const envDir = process.env.CODEX_HOME || process.env.CLAUDE_CONFIG_DIR;
  if (envDir && fs.existsSync(path.join(envDir, 'get-shit-done', 'VERSION'))) {
    return envDir;
  }
  for (const dir of ['.codex', '.config/opencode', '.opencode', '.claude']) {

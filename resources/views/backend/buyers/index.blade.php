<div>
  ...
  <div class="bg-white shadow-sm rounded-lg mb-6 p-4">
    <form action="{{ route('backend.buyers.index') }}" method="GET" class="flex flex-wrap gap-4">
      <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="{{ __('common_search') }}" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
      <input type="number" name="min_balance" id="min_balance" value="{{ request('min_balance') }}" placeholder="{{ __('common_min_balance') }}" class="w-48 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
      <input type="number" name="max_balance" id="max_balance" value="{{ request('max_balance') }}" placeholder="{{ __('common_max_balance') }}" class="w-48 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
  ...
  <div class="bg-white shadow-sm sm:rounded-lg">
    <table class="w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_name') }}</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_email') }}</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_company_name') }}</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_credit_balance') }}</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_verification_status') }}</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('buyers_field_active_status') }}</th>
        ...
        @foreach ($buyers as $buyer)
          <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
              <a href="{{ route('backend.buyers.credit', $buyer) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('common_balance') }}</a>

              <a href="{{ route('backend.buyers.orders', $buyer) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('common_orders') }}</a>
              <a href="{{ route('backend.buyers.edit', $buyer) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('common_edit') }}</a>
              <x-button
                  xs
                  flat
                  negative

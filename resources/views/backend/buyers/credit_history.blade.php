                ...
                <div class="flex items-center mb-6">
                    <div class="ml-6">
                        ...
                        <div class="mt-1">
                            ...
                            <span class="ml-2 text-sm text-gray-500">{{ __('backend.buyers.credit.current_balance') }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="GET" class="flex items-center space-x-4">
                <!-- Type Filter -->
                <div class="flex-1">
                    <label for="type" class="block text-sm font-medium text-gray-700">{{ __('backend.buyers.credit_history.filter.type') }}</label>
                ...
                <!-- Date From -->
                <div class="flex-1">
                    <label for="date_from" class="block text-sm font-medium text-gray-700">{{ __('backend.buyers.credit_history.filter.date_from') }}</label>
                ...
                <!-- Date To -->
                <div class="flex-1">
                    <label for="date_to" class="block text-sm font-medium text-gray-700">{{ __('backend.buyers.credit_history.filter.date_to') }}</label>
                ...
                @forelse ($creditHistory as $history)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($history->type === 'add')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    {{ __('backend.buyers.credit_history.table.credit') }}
                                </span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                    {{ __('backend.buyers.credit_history.table.debit') }}
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $history->admin ? $history->admin->name : __('backend.buyers.credit_history.table.system') }}
                        </td>
                    </tr>
                    <tr class="hover:bg-gray-50">

...
@section('content')
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="bg-green-100 text-green-700 p-4 rounded mb-4 success-import hidden">
            <p><?php echo __('translation.done_importing', ['count' => '<strong class="counter">N</strong>']); ?> <?php echo __('translation.reload_page'); ?></p>
        </div>
        <div class="bg-green-100 text-green-700 p-4 rounded mb-4 success-find hidden">
            <p><?php echo __('translation.done_searching', ['count' => '<strong class="counter">N</strong>']); ?></p>
        </div>
        <div class="bg-green-100 text-green-700 p-4 rounded mb-4 success-publish hidden">
            <p><?php echo __('translation.done_publishing_group', ['group' => $group]); ?></p>
        </div>
        <div class="bg-green-100 text-green-700 p-4 rounded mb-4 success-publish-all hidden">
        ...
        <div class="mb-8">
            <?php if(!isset($group)) : ?>
            <form class="form-find mb-4" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind'); ?>" data-remote="true" role="form"
            ...
            <?php if(isset($group)) : ?>
            <form class="inline-block form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group); ?>" data-remote="true"
                ...
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>"
                    class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded"><?php echo __('translation.back'); ?></a>
            </form>
            <?php endif; ?>
        </div>

        <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup'); ?>" class="mb-8">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="mb-4">
                <p class="text-gray-700 mb-2"><?php echo __('translation.choose_group'); ?></p>
                <select name="group" id="group"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-blue-500 group-select">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white">
                <thead class="sticky top-0 bg-white">
                    <tr>
                        <th
                            class="w-1/6 px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <?php echo __('translation.key'); ?></th>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900"><?php echo __('translation.supported_locales'); ?></h3>
                ...
                <form class="form-remove-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postRemoveLocale'); ?>"
                    ...
                    <ul class="divide-y divide-gray-200">
                        ...
                        <li class="py-4 flex">
                            ...
                            <span class="ml-3"><?php echo $locale; ?></span>
                        </li>
                        <?php endforeach; ?>
                    </ul>
                </form>
        ...
        <div class="mt-8">
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*'); ?>" data-remote="true"
                role="form" data-confirm="<?php echo __('translation.confirm_publish_all'); ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"

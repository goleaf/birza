...
class UpdateSellerDiscountAction
{
     ...
     */
    public function handle(Seller $seller, Discount $discount, array $attributes): Discount
    {
        Gate::forUser($seller)->authorize('update', $discount);

        $oldValues = $this->auditLogsAction->discountSnapshot($discount);
        $discount->update($this->validated($seller, $attributes));
        $discount->refresh();

        $this->auditLogsAction->discountUpdated($seller, $discount, $oldValues, 'seller_discount_update');

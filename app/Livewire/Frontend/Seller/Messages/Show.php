...
class Show extends Component
{
    ...
    public function mount(Conversation $conversation): void
    {
        ...
        app(MarkConversationAsReadAction::class)->handle($this->conversation, $this->seller());
    }

    public function sendMessage(SendMessageAction $action): void
    {
        try {
            $action->handle($this->conversation->fresh() ?? $this->conversation, $this->seller(), $this->body);
        ...
        $this->body = '';
        $this->conversation = $this->conversation->refresh()->load(['buyer:id,name,company_name', 'product:id,name', 'order:id']);
        session()->flash('success', __('messages.sent_successfully'));
    }

    public function archive(ArchiveConversationAction $action): void
    {
        $action->handle($this->conversation, $this->seller());

        session()->flash('success', __('messages.archived_successfully'));
        $this->redirectRoute('seller.messages.index', navigate: true);
    }
    ...
    public function render(): View
    {
        ...
        return view('livewire.frontend.seller.messages.show', [
            'actor' => $this->seller(),
            'messages' => $messages,
        ]);
    }

# Documentation Index

This folder contains project-owned documentation for Birza.

## Start Here

- [Installation guide](installation.md)
- [Environment guide](environment.md)
- [Architecture guide](architecture.md)
- [Roles and access guide](roles.md)
- [Database guide](database.md)
- [Frontend guide](frontend.md)
- [Testing guide](testing.md)
- [Seeders and factories guide](seeders.md)
- [Security guide](security.md)
- [Performance guide](performance.md)
- [Roadmap](roadmap.md)
- [Developer workflow](developer-workflow.md)
- [Production notes](production.md)
- [Documentation audit - 2026-06-07](documentation-audit-2026-06-07.md)

## Release Management

- [Release workflow](releases/README.md)
- [Release notes template](release-notes-template.md)
- [Changelog](../CHANGELOG.md)
- [Initial baseline release notes](releases/0.1.0.md)
- [Release notes 0.2.0](releases/0.2.0.md)
- [Release notes 0.3.0](releases/0.3.0.md)

## Feature And System Guides

- [Audit log system](audit-log-system.md)
- [Cart and checkout workflow](cart-checkout-workflow-2026-06-07.md)
- [Demo seeding guide](demo-seeding.md)
- [Deployment notes](deployment.md)
- [Image pipeline](image-pipeline.md)
- [Marketplace notifications](notifications.md)
- [Product reports and abuse moderation](product-reports.md)
- [Product wishlists](product-wishlists.md)
- [Stock alerts](stock-alerts.md)
- [Translation guide](translations.md)

## Technical Audits

- [Database structure audit - 2026-06-07](database-structure-audit-2026-06-07.md)
- [Eloquent relationship map - 2026-06-07](eloquent-relationship-map-2026-06-07.md)
- [Foreign key constraint audit - 2026-06-07](foreign-key-constraint-audit-2026-06-07.md)
- [Frontend stack compatibility report - 2026-06-07](frontend-stack-compatibility-2026-06-07.md)
- [Query index audit - 2026-06-07](query-index-audit-2026-06-07.md)
- [Security and authorization audit](security-authorization.md)

Some audit files are historical snapshots. Prefer the current guide files above for day-to-day implementation decisions.

## Module Release Notes

- [Unreleased product questions](releases/unreleased-product-questions.md)
- [Unreleased product reports](releases/unreleased-product-reports.md)
- [Unreleased product wishlists](releases/unreleased-product-wishlists.md)
- [Unreleased seller discounts and promo codes](releases/unreleased-seller-discounts-promo-codes.md)

## Screenshots

Screenshots should live in `docs/screenshots/`. The directory is present but the full screenshot set is still TODO.
